...
class DocumentViewModel : ViewModel() {
    ...
    // ฟังก์ชันอัปโหลดไฟล์
    fun uploadFile(context: Context, userId: Int, uri: Uri, year: Int, filename: String) {
        viewModelScope.launch {
            try {
                _isLoading.value = true
                ...
                // สร้าง RequestBody สำหรับชื่อไฟล์
                val documentnameBody = filename.toRequestBody("text/plain".toMediaTypeOrNull())

                // เรียก API เพื่ออัปโหลดไฟล์
                api.uploadDocument(userId, documentnameBody, filePart, yearBody)
    ...
    fun updateDocumentName(userId: Int, fileId: Int, newName: String) {
        viewModelScope.launch {
            try {
                _isLoading.value = true

                // Call the API with correct parameters
                val response = api.updateDocumentName(fileId, userId, newName)

                // Mark as successful
                _updateSuccess.value = true

                // After successful update, refresh the documents list
                fetchDocumentsById(fileId)

                // Optional: You might want to update the local data immediately
                // to avoid waiting for the re-fetch
                _documents.value = _documents.value.map {
                    if (it.id == fileId) it.copy(documentName = newName) else it
                }

            } catch (e: Exception) {
                e.printStackTrace()
                _errorMessage.value = "อัปเดตชื่อเอกสารล้มเหลว: ${e.message}"
                // Make sure update is not considered successful on error
                _updateSuccess.value = false
            } finally {
                _isLoading.value = false
            }
